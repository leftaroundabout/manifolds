...
class (WithField ℝ Manifold y) => Refinable y where
  ...
  -- = ⟨cc|e₁|cc⟩ − ⟨c₁|e₁|cc⟩ − ⟨cc|e₁|c₁⟩ + ⟨c₁|e₁|c₁⟩ + ⟨cc|e₂|cc⟩ − ⟨c₂|e₂|cc⟩ − ⟨cc|e₂|c₂⟩ + ⟨c₂|e₂|c₂⟩
  -- It is thus
  -- β(cc + δ⋅v) − β cc
  -- = ⟨cc+δ⋅v| e₁ |cc+δ⋅v⟩ − ⟨c₁| e₁ |cc+δ⋅v⟩ − ⟨cc+δ⋅v| e₁ |c₁⟩ + ⟨cc+δ⋅v| e₂ |cc+δ⋅v⟩ − ⟨c₂| e₂ |cc+δ⋅v⟩ − ⟨cc+δ⋅v| e₂ |c₂⟩
  --    − ⟨cc|e₁|cc⟩ + ⟨c₁|e₁|cc⟩ + ⟨cc|e₁|c₁⟩ − ⟨cc|e₂|cc⟩ + ⟨c₂|e₂|cc⟩ + ⟨cc|e₂|c₂⟩
  -- = δ⋅( ⟨cc|e₁|v⟩ + ⟨v|e₁|cc⟩ − ⟨c₁|e₁|v⟩ − ⟨v|e₁|c₁⟩ + ⟨cc|e₂|v⟩ + ⟨v|e₂|cc⟩ − ⟨c₂|e₂|v⟩ − ⟨v|e₂|c₂⟩ )
  --   + 𝓞(δ²)
  -- We need for all v that
  -- 0 = ⟨cc|e₁|v⟩ + ⟨v|e₁|cc⟩ − ⟨c₁|e₁|v⟩ − 
  --      + ⟨cc|e₂|v⟩ + ⟨v|e₂|cc⟩ − ⟨c₂|e₂|v⟩ − ⟨v|e₂|c₂⟩
  ...
  -- ⟨x−cc|ee|x−cc⟩ ⋅ α
  --  = ⟨x−(c₁+c')|ee|x−(c₁+c')⟩ ⋅ α
  --  = α⋅⟨x−c₁|ee|x−c₁−c'⟩ − α⋅⟨c'|ee|x−c₁−c'⟩
  --  = α⋅⟨x−c₁|ee|x−c₁⟩ − α⋅⟨x−c₁|ee|c'⟩ − α⋅⟨c'|ee|x−c₁⟩ + α⋅⟨c'|ee|c'⟩
  --  = ⟨x−c₁|e₁+e₂|x−c₁⟩ − ⟨x−c₁|e₁+e₂|c'⟩ − ⟨c'|e₁+e₂|x−c₁⟩ + ⟨c'|e₁+e₂|c'⟩
  --  = ⟨x−c₁|e₁+e₂|x−c₁⟩ + (-2⋅⟨x−c₁| + ⟨c'|) (e₁+e₂) |c'⟩
  --  = ⟨x−c₁|e₁+e₂|x−c₁⟩ + (-2⋅⟨x−c₁| + ⟨c'|) (e₂|c₂−c₁⟩)
  --  = ⟨x−c₁|e₁+e₂|x−c₁⟩ − 2⋅⟨x−c₁|e₂|c₂−c₁⟩ + ⟨c'|e₂|c₂−c₁⟩
  ...
  --    − 2⋅⟨x−c₁|e₂|c₂−c₁⟩ + ⟨c'|e₂|c₂−c₁⟩
  --  = 2 + ⟨c₁|e₂|c₁⟩ − ⟨c₂|e₂|c₂⟩
  --      + 2⋅⟨c₂−c₁|e₂|x⟩ − 2⋅⟨x−c₁|e₂|c₂−c₁⟩ + ⟨c'|e₂|c₂−c₁⟩
  --  = 2 + ⟨c₁|e₂|c₁⟩ − ⟨c₂|e₂|c₂⟩
  --      + 2⋅⟨x|e₂|c₂−c₁⟩ + 2⋅⟨c₁−x|e₂|c₂−c₁⟩ + ⟨c'|e₂|c₂−c₁⟩
  --  = 2 + ⟨c₁|e₂|c₁⟩ − ⟨c₂|e₂|c₂⟩
  --       + 2⋅⟨c₁|e₂|c₂−c₁⟩ + ⟨c'|e₂|c₂−c₁⟩
  --  = 2 − ⟨c₂|e₂|c₂⟩ + 2⋅⟨c₁|e₂|c₂−c₁⟩ + ⟨c₁|e₂|c₂⟩ + ⟨c'|e₂|c₂−c₁⟩
  --  = 2 + ⟨c₁−c₂|e₂|c₂⟩ + 2⋅⟨c₁|e₂|c₂−c₁⟩ + ⟨c'|e₂|c₂−c₁⟩
  --  = 2 − ⟨c₂|e₂|c₂−c₁⟩ + 2⋅⟨c₁|e₂|c₂−c₁⟩ + ⟨c'|e₂|c₂−c₁⟩
  --  = 2 + ⟨c₁−c₂|e₂|c₂−c₁⟩ + ⟨c₁|e₂|c₂−c₁⟩ + ⟨c'|e₂|c₂−c₁⟩
  --  = ???????? 2 + ⟨c₂−c₁|e₂|c₂−c₁⟩ + ⟨cc|e₂|c₂−c₁⟩
  --  = 2 + |δc|◞e₂ + ⟨c'|e₂|δc⟩
  --  ≡ α

  -- ⟨x−cc|ee|x−cc⟩ ⋅ α
  --  = ⟨x|e₁+e₂|x⟩ + (-2⋅⟨x| + ⟨cc|) (e₁+e₂) |cc⟩
  --  = ⟨x|e₁+e₂|x⟩ + (-2⋅⟨x| + ⟨cc|) (e₁|c₁⟩ + e₂|c₂⟩)
  --  = ⟨x|e₁|x⟩ − 2⋅⟨x|e₁|c₁⟩ + ⟨cc|e₁|c₁⟩
  --   + ⟨x|e₂|x⟩ − 2⋅⟨x|e₂|c₂⟩ + ⟨cc|e₂|c₂⟩
  --  = ⟨x|e₁|x⟩ − 2⋅⟨x|e₁|c₁⟩ + (1-1)⋅⟨c₁|e₁|c₁⟩ + ⟨cc|e₁|c₁⟩
  --   + ⟨x|e₂|x⟩ − 2⋅⟨x|e₂|c₂⟩ + (1-1)⋅⟨c₂|e₂|c₂⟩ + ⟨cc|e₂|c₂⟩
  --  = ⟨x−c₁|e₁|x−c₁⟩ − ⟨c₁|e₁|c₁⟩ + ⟨cc|e₁|c₁⟩
  --   + ⟨x−c₂|e₂|x−c₂⟩ − ⟨c₂|e₂|c₂⟩ + ⟨cc|e₂|c₂⟩
  --  ≤ 1 + ⟨cc−c₁|e₁|c₁⟩ + 1 + ⟨cc−c₂|e₂|c₂⟩
  --  = 2 + ⟨cc−c₁|e₁|c₁⟩ + ⟨cc−c₂|e₂|c₂⟩
  --  = 2 + ⟨cc−c₁|e₁|cc−c₁⟩ − ⟨cc−c₁|e₁|cc−2⋅c₁⟩ + ⟨cc−c₂|e₂|c₂⟩
